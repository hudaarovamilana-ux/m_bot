...
# ВСТАВЬТЕ ТОКЕН (для теста можно временно вставить сюда)
TOKEN = "8747157657:AAFV3g0pxEVi8a5_6R5QJNzSaQgCC6WpWKs"  # Или замените на "ваш_токен"
...
async def process_week(message: types.Message, state: FSMContext):
    try:
        week = int(message.text)
        if 1 <= week <= 42:
            # Рассчитываем дату родов
            today = datetime.now()
            days_from_start = week * 7
            conception_date = today - timedelta(days=days_from_start)
            due_date = conception_date + timedelta(days=280)
            
            # 👇🏻 ВОТ ЭТОТ КУСОК ДОЛЖЕН БЫТЬ!
            from database import add_user
            print(f"🔍 Сохраняю пользователя {message.from_user.id} с неделей {week}")
            add_user(
                user_id=message.from_user.id,
                week=week,
                due_date=due_date.strftime('%Y-%m-%d')
...
async def process_date(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if weeks_passed < 1:
            await message.answer("🌸 Поздравляю с началом беременности!...")
            await state.clear()
            return
        ...
        from database import add_user
        print(f"🔍 Сохраняю пользователя {message.from_user.id} с неделей {weeks_passed}")
        add_user(
            user_id=message.from_user.id,
            week=weeks_passed,
            due_date=due_date.strftime('%Y-%m-%d'),
        ...
        await message.answer(
            f"📊 По моим расчетам:\n"
            f"Срок беременности: {weeks_passed} недель {days_passed % 7} дней\n"
            f"📅 Примерная дата родов: {due_date.strftime('%d.%m.%Y')}\n\n"
            f"🔔 Я буду напоминать тебе о новой неделе каждые 7 дней!\n\n"
            f"✨ Вот информация для {weeks_passed} недели:"
        )
...
async def back_to_main_menu(callback: types.CallbackQuery):
    ...
    await callback.answer()
@dp.message(lambda message: message.text == "📋 Анализы")
async def show_analyses_menu(message: types.Message):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await message.answer("📋 Выбери триместр:", reply_markup=keyboard)
...
async def show_third_trimester_analyses(callback_query: types.CallbackQuery):
    ...
    await callback_query.answer()
# В самый конец файла, перед запуском бота:
async def main():
    # База уже инициализирована выше, но можно вызвать ещё раз для надежности
    init_db()
