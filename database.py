...
def get_connection():
    ...
    conn = sqlite3.connect('pregnancy_bot.db')
    cursor = conn.cursor()
...
def start_kick_count(user_id):
    ...
    try:
        ...
        print(f"❌ Ошибка в start_kick_count: {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def get_today_kicks(user_id):
    ...
    try:
        ...
        print(f"❌ Ошибка в get_today_kicks: {e}")
        return 0
    finally:
        if conn:
            conn.close()
...
def get_kick_history(user_id, days=7):
    ...
    try:
        ...
        print(f"❌ Ошибка в get_kick_history: {e}")
        return []
    finally:
        if conn:
            conn.close()